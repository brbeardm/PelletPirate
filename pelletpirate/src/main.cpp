...
//void CalcPID()
//{
//    if((Time.now() - LCalcula) > PIDCycleTime )
//    {
//        u = myPID.update(Temps[0]);
//        if(u == 0){u=0.15;};
//        Serial.printf("CalcPID - PID: %.2f\r\n", u);
//        LCalcula = Time.now();
//    }
//}
...
{
    int TimeSinceWrite = Time.now() - LWritten;

    if (TimeSinceWrite > ParametersInterval && ResetFIREBASE == 1) // gotta make sure the firebase delete has had time to finish
    {
