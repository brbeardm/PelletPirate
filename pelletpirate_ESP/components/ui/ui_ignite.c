...
// 2. Cook Mode shown as "Ignite", focus on Start Ignite button
...
static void do_ignite(lv_event_t *e)
{
    ...
    if (gs->grill_temp >= IGNITE_DISABLE_TEMP) {
        ESP_LOGW(TAG, "Grill too hot (%.0f°F) — ignite disabled", gs->grill_temp);
        grill_state_unlock();
        return;
    }
...
lv_obj_t *ui_ignite_create(void)
{
    ...
    // Cook Mode — show current mode, not "Ignite" until confirmed
    grill_state_lock();
    grill_mode_t cur_mode = grill_state_get()->mode;
    grill_state_unlock();
    ...
    lbl = lv_label_create(s_screen);
    lv_label_set_text_fmt(lbl, "%.0f\xC2\xB0""F", cur_temp);
    ...
    if (too_hot) {
        ...
        lbl = lv_label_create(s_screen);
        lv_label_set_text(lbl, "Press START IGNITE to begin\nignition sequence");
        lv_obj_set_style_text_font(lbl, &lv_font_montserrat_16, 0);
    ...
    // Start Ignite button (or disabled if too hot)
    if (!too_hot) {
        lv_obj_t *btn_ign = lv_button_create(s_screen);
        lv_obj_set_size(btn_ign, 150, 36);
